...
def ode_dfdx(t, x, u, p, ode_args):
    ...
    return [
        [
            mu_max_x * (x2/(x2 + 1)) * (1 - 2*x1/xy_max) - alpha_x * y1 / xy_max, # df1/dx1
            mu_max_x * 1/(x2 + 1)**2 * (1 - x1/xy_max)*x1,                        # df1/dx2
            - alpha_x * x1 / xy_max,                                              # df1/dy1
            0,                                                                    # df1/dy2
        ],
        [
            0,                                                                    # df2/dx1
        ...
        [
            - alpha_x * y1 / xy_max,                                              # df3/dx1
            0,
            mu_max_y * (y2/(y2 + 1)) * (1 - 2*y1/xy_max) - alpha_y * x1 / xy_max, # df3/dy1
            mu_max_y * 1/(y2 + 1)**2 * (1 - y1/xy_max)*y1,                        # df3/dy2
        ],
        [
            0,                                                                    # df4/dx1
            0,                                                                    # df4/dx2
...
class ModelExtendedBaranyi:
    def __init__(self, obs=[0], N_x0=2, n_t0=13, n_times=5, n_inputs=7, identical_times=False):
        ...
        self.parameters = (1e8, 0.2, 1.0, 3.5, 0.2, 1.0, 3.5)
