...
def plot_all_odes(fsr: FisherResults, outdir=Path(".")):
    for i, sol in enumerate(fsr.individual_results):
        ...
        times_high = fsr._fsm_var_def.times.ub if fsr._fsm_var_def.times is not None else np.max(sol.times)
...
def plot_all_sensitivities(fsr: FisherResults, outdir=Path(".")):
    for i, sol in enumerate(fsr.individual_results):
        ...
        res = sp.integrate.solve_ivp(ode_rhs, (times_low, times_high), y0_full, t_eval=t_values, args=(fsr.ode_fun, fsr.ode_dfdx, fsr.ode_dfdp, sol.inputs, sol.parameters, sol.constants, n_x, n_p))
        t = np.array(res.t)
