...
def plot_all_odes(fsr: FisherResults, outdir=Path(".")):
    for i, sol in enumerate(fsr.individual_results):
        ...
        res = sp.integrate.solve_ivp(fsr.ode_fun, (times_low, times_high), sol.ode_y0, t_eval=t_values, args=(sol.inputs, sol.parameters, sol.ode_args))
        t = np.array(res.t)
        y = np.array(res.y)
        ax.plot(t, y.T, color="#21918c", label="Ode Solution")

        # Determine where multiple time points overlap by rounding
        ax.scatter(sol.ode_solution.t, sol.ode_solution.y[:len(sol.ode_y0)], s=160, alpha=0.5, color="#440154", label="Q_values: " + str(sol.inputs))
        ax.legend()
...
def plot_all_sensitivities(fsr: FisherResults, outdir=Path(".")):
    for i, sol in enumerate(fsr.individual_results):
        ...
        y0_full = np.concatenate((sol.ode_y0, np.zeros(n_x * n_p)))
        res = sp.integrate.solve_ivp(ode_rhs, (times_low, times_high), y0_full, t_eval=t_values, args=(fsr.ode_fun, fsr.ode_dfdx, fsr.ode_dfdp, sol.inputs, sol.parameters, sol.ode_args, n_x, n_p))
        t = np.array(res.t)
